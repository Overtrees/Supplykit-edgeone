...
export default function RulesPage() {
  ...
  const loadSlowCfg = async () => { try { const c=globalChannel; const [rc,rf]=await Promise.all([api.get('/api/replenishment-config/slow-cats?channel='+c),api.get('/api/replenishment-config?channel='+c)]); if(rc.data)setSlowCats(Array.isArray(rc.data)?rc.data:[]); if(rf.data){ if(rf.data.transit_days)setTransitDays(rf.data.transit_days); if(rf.data.slow_fund_threshold)setFundThreshold(rf.data.slow_fund_threshold) } } catch(e) {} }
  ...
  const resetForm = () => { setEditing({}); setF({...defaultF, mode: hammerRulesMode || (globalChannel === 'jd' ? 'bbcc' : 'traditional')}); setRParams({}); setCond({left:'inv.available_qty', op:'<', right:'inv.safety_qty', rightType:'field', pctValue:100, warehouse:'', or:[]}) }
  const cancelEdit = () => { setEditing(null); setF({...defaultF, mode: hammerRulesMode || (globalChannel === 'jd' ? 'bbcc' : 'traditional')}); setRParams({}); setCond({left:'inv.available_qty', op:'<', right:'inv.safety_qty', rightType:'field', pctValue:100, warehouse:'', or:[]}); useAppStore.setState({ hammerRuleNewVersion: 0 }) }
  ...
  const save = async () => {
    ...
    try {
      ...
      const r = await fetch(url, {method: isNew?'POST':'PUT', headers:{'Authorization':'Bearer '+(()=>{try{return localStorage.getItem('c_token')}catch{return ''}})(), 'Content-Type':'application/json'}, body:JSON.stringify({...f, mode: f.mode || (editing && editing.mode) || '', channel:globalChannel, condition_json:cj, params:rParams})})

...
<div className='section-title' style={{display:'flex',flexWrap:'wrap',gap:6,alignItems:'center'}}>
  <span>商品管理 <span className='small muted' style={{fontSize:'var(--font-xs)',fontWeight:400}}>已加载 {Math.min(list.length, pgTotal||list.length)}/{pgTotal||list.length} 条 · 显示 {visCols.length}/{COLS.length} 列{s ? ` · "${s}"` : ''}</span></span>
</div>
...
<tbody>{fl.map(x=><tr key={x.id} style={{...(prodBatch&&selIds.includes(x.id)?{background:'rgba(29,78,216,0.08)'}:undefined),...(x.status!=='active'?{opacity:0.55}:undefined)}}>{prodBatch&&<td className="clickable" onClick={(e)=>{const ids=selIds;setSelIds(ids.includes(x.id)?ids.filter(i=>i!==x.id):[...ids,x.id])}}><span style={{width:18,height:18,borderRadius:'var(--radius-xs)',border:'1.5px solid',borderColor:selIds.includes(x.id)?'var(--primary)':'var(--border)',background:selIds.includes(x.id)?'var(--primary)':'transparent',display:'inline-flex',alignItems:'center',justifyContent:'center',color:'#fff',fontSize:'var(--font-xs)'}}>{selIds.includes(x.id)?<IconCheck size={12} />:''}</span></td>}{visCols.map(id=>{const col=COLS.find(c=>c.id===id);if(!col)return <td key={col.id} className="small muted" style={{fontSize:'var(--font-xs)'}}>-</td>;
  if(col.id==='barcode')return <td key={col.id} className='mono' style={{fontSize:'var(--font-xs)'}}>{x.barcode||'-'}</td>
  if(col.id==='channel')return <td key={col.id} style={{fontSize:'var(--font-xs)'}}>{(useAppStore.getState().channel)==='other'?'其他':'京东'}</td>
  ...
  if(col.id==='box')return <td key={col.id} className='col-qty' style={{fontSize:'var(--font-sm)'}}>{x.box_qty||1}</td>
  if(col.id==='unit')return <td key={col.id} className='col-qty' style={{fontSize:'var(--font-sm)'}}>{x.unit||'瓶'}</td>
  if(col.id==='weight')return <td key={col.id} className='col-qty' style={{fontSize:'var(--font-sm)'}}>{x.weight||'-'}</td>
  if(col.id==='volume')return <td key={col.id} className='col-qty' style={{fontSize:'var(--font-sm)'}}>{x.volume||'-'}</td>
  if(col.id==='batch_days')return <td key={col.id} className='col-qty' style={{fontSize:'var(--font-sm)'}}>{x.batch_days||'-'}</td>
  ...
  return <td key={col.id} className="small muted" style={{fontSize:'var(--font-xs)'}}>-</td>
        ...
        {pgTotal > 0 && list.length < pgTotal && (
          ...
          }}><span className="btn btn-ghost" style={{fontSize:'var(--font-sm)',padding:'6px 16px',cursor:'pointer'}}>{loadingMore ? '加载中... ' : ''}({Math.min(list.length, pgTotal)}/{pgTotal})</span></div>
        )}
        {pgTotal > 0 && list.length >= pgTotal && <div style={{textAlign:'center',padding:'10px 0',fontSize:'var(--font-xs)',color:'var(--muted2)'}}>已加载全部 {pgTotal} 条</div>}
</div>}
</div>}
...
<tbody>{fl.map(x=><tr key={x.id} style={{...(prodBatch&&selIds.includes(x.id)?{background:'rgba(29,78,216,0.08)'}:undefined),...(x.status!=='active'?{opacity:0.55}:undefined)}}>{prodBatch&&<td className="clickable" onClick={(e)=>{const ids=selIds;setSelIds(ids.includes(x.id)?ids.filter(i=>i!==x.id):[...ids,x.id])}}><span style={{width:18,height:18,borderRadius:'var(--radius-xs)',border:'1.5px solid',borderColor:selIds.includes(x.id)?'var(--primary)':'var(--border)',background:selIds.includes(x.id)?'var(--primary)':'transparent',display:'inline-flex',alignItems:'center',justifyContent:'center',color:'#fff',fontSize:'var(--font-xs)'}}>{selIds.includes(x.id)?'✓':''}</span></td>}{visCols.map(id=>{const col=COLS.find(c=>c.id===id);if(!col)return <td key={col.id} className="small muted" style={{fontSize:'var(--font-xs)'}}>-</td>;

      ...
      <tbody>{fl.map(x => {
        ...
        if(isOpen&&batchLoading[bk]){
          ...
          batchData[bk].forEach(function(b,bi){
            ...
            const bcells=visCols.map(function(id){const col=INV_COLS[whType].find(function(c){return c.id===id});if(!col)return null;let el;if(col.id==='prod_date')el=React.createElement('td',{key:col.id,style:{fontSize:11}},b.prod_date||'-');else if(col.id==='exp_date')el=React.createElement('td',{key:col.id,style:{fontSize:11}},b.exp_date||'-');else if(col.id==='batch_days'){let td=0;if(b.exp_date&&b.prod_date)td=Math.round((new Date(b.exp_date)-new Date(b.prod_date))/86400000);el=React.createElement('td',{key:col.id,style:{fontSize:11}},td||'-')}else if(col.id==='eff_status'){let es='';if(b.exp_date&&b.prod_date){const cv=Math.round(((new Date()-new Date(b.prod_date))/86400000)/((new Date(b.exp_date)-new Date(b.prod_date))/86400000)*100);es=cv>=100?'⚫过期':(cv>=67?'✗否':(cv>40?'⚠️临近':'✓正常'))}el=React.createElement('td',{key:col.id,style:{fontSize:11,fontWeight:600,color:es==='✓正常'?'var(--success)':es==='⚠️临近'?'var(--warning)':es==='✗否'?'var(--danger)':es==='⚫过期'?'#7c3aed':'var(--muted2)'}},es)}else if(col.id==='avail')el=React.createElement('td',{key:col.id,style:{fontWeight:600,fontSize:11}},b.qty);else if(col.id==='over_third'){let ot_st='-';if(b.exp_date&&b.prod_date){const td=Math.round((new Date(b.exp_date)-new Date(b.prod_date))/86400000);if(td>0){const cv=Math.round(((new Date()-new Date(b.prod_date))/86400000)/td*100);ot_st=cv>=100?'⚫已过期':(cv>33?'✗ 已超1/3':'正常')}}el=React.createElement('td',{key:col.id,style:{fontSize:11,color:ot_st.includes('已超')?'var(--danger)':ot_st.includes('过期')?'#7c3aed':'var(--muted2)'}},ot_st)}else if(col.id==='note')el=React.createElement('td',{key:col.id,style:{fontSize:11,color:bcolor,fontWeight:600}},pct+'% '+note);else if(col.id==='warehouse'||col.id==='sku'||col.id==='name'||col.id==='barcode'||col.id==='channel'||col.id==='brand'){const v=col.id==='name'?x.product_name:(col.id==='brand'?(x.brand||'-'):(x[col.id]||'-'));el=React.createElement('td',{key:col.id,style:{fontSize:11}},v)}else if(col.id==='price')el=React.createElement('td',{key:col.id,style:{fontSize:11}},x.price?('¥'+Number(x.price).toFixed(1)):'-');else if(col.id==='month_in')el=React.createElement('td',{key:col.id,style:{fontSize:11,color:'var(--muted2)'}},'-')
        ...
        }        const _hb=(x.batch_count||0)>1;return [React.createElement('tr',{key:x.id,id:'hl-'+x.sku+'-'+(x.warehouse||''),onClick:_hb?function(){toggleBatch(x)}:null,className:_hb?(isOpen?'tr-click tr-open':'tr-click'):'',style:isHL?{background:'rgba(245,158,11,0.15)',outline:'2px solid #f59e0b'}:{}},visCells)].concat(batchTrs)      })}
      </tbody>

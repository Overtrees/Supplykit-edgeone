  ...
  return <div className='card'>
    ...
    : fl.length === 0
      ...
      <tbody>{fl.map(x => {
        ...
        const visCells = visCols.map(function(id){const col=INV_COLS[whType].find(function(c){return c.id===id});if(!col)return null;let el;if(col.id==='warehouse'){const isOpen=batchOpen.includes(x.sku+'|'+x.warehouse);const hasBatch=(x.batch_count||0)>1;el=React.createElement('td',{key:col.id,className:'col-store'},x.warehouse||'-',hasBatch?React.createElement('span',{style:{fontSize:10,marginLeft:6,color:'var(--primary)'}},isOpen?'▴':'⤵ 批次'):null);}else if(col.id==='channel')el=React.createElement('td',{key:col.id,style:{fontSize:11}},x.channel==='other'?'其他':'京东');else if(col.id==='brand')el=React.createElement('td',{key:col.id,style:{fontSize:11}},x.brand||'-');else if(col.id==='sku')el=React.createElement('td',{key:col.id,className:'mono col-sku'},x.sku);else if(col.id==='barcode')el=React.createElement('td',{key:col.id,className:'mono',style:{fontSize:11}},x.barcode||'-');else if(col.id==='name')el=React.createElement('td',{key:col.id,className:'col-name'},x.product_name);else if(col.id==='begin'){const _beg=(x.available_qty||0)-(x.month_inbound||0)+(x.month_outbound||0);el=React.createElement('td',{key:col.id,className:'col-qty',style:{fontWeight:600}},_beg)}else if(col.id==='transit')el=React.createElement('td',{key:col.id,className:'col-qty'},x.in_transit_qty);else if(col.id==='c_transit')el=React.createElement('td',{key:col.id,className:'col-qty'},x.c_transit||0);else if(col.id==='month_in')el=React.createElement('td',{key:col.id,className:'col-qty'},x.month_inbound??0);else if(col.id==='month_out')el=React.createElement('td',{key:col.id,className:'col-qty',style:{fontWeight:600}},x.month_outbound??0);else if(col.id==='prod_date')el=React.createElement('td',{key:col.id,className:'col-qty',style:{fontSize:11}},x.batch_prod_date||'-');else if(col.id==='exp_date')el=React.createElement('td',{key:col.id,className:'col-qty',style:{fontSize:11}},x.batch_exp_date||'-');else if(col.id==='batch_days')el=React.createElement('td',{key:col.id,className:'col-qty',style:{fontSize:11}},x.batch_days||'-');else if(col.id==='eff_status'){const es=x.batch_status;const ecolor=es==='ok'?'var(--success)':es==='warn'?'var(--warning)':es==='no'?'var(--danger)':(es==='expired'?'var(--accent-purple)':'var(--muted2)');const elbl=es==='ok'?'✓ 正常':es==='warn'?'⚠️ 临近':es==='no'?'✗ 否':(es==='expired'?'⚫ 过期':'-');el=React.createElement('td',{key:col.id},React.createElement('span',{style:{fontSize:11,fontWeight:600,color:ecolor}},elbl),x.batch_pct?React.createElement('span',{style:{fontSize:10,color:'var(--muted2)',marginLeft:4}},x.batch_pct+'%'):null);}else if(col.id==='over_third'){const _ot_st=x.batch_status;el=React.createElement('td',{key:col.id,style:{fontSize:11}},_ot_st==='no'?React.createElement('span',{style:{color:'var(--danger)',fontWeight:600}},'✗ 已超1/3'):_ot_st==='expired'?React.createElement('span',{style:{color:'var(--accent-purple)'}},'已过期'):_ot_st==='warn'?React.createElement('span',{style:{color:'var(--warning)',fontWeight:600}},'⚠️ 临近'):_ot_st==='ok'?React.createElement('span',{style:{color:'var(--muted2)'}},'否'):'-');}else if(col.id==='note'){const _ns=x.batch_status;const _nb=(x.batch_pct||0);const _nclr=_ns==='ok'?'var(--success)':_ns==='warn'?'var(--warning)':_ns==='no'?'var(--danger)':(_ns==='expired'?'var(--accent-purple)':'var(--muted2)');const _nlbl=_ns==='ok'?'正常':_ns==='warn'?'临近1/3':_ns==='no'?'已超1/3':(_ns==='expired'?'已过期':'');const _nact=_ns==='ok'?(_nb>30?'动销放缓，促动销':'正常销售'):_ns==='warn'?'促销去库/调拨':_ns==='no'?'尽快清仓/退供':(_ns==='expired'?'报废/退供应商':'');el=React.createElement('td',{key:col.id,style:{fontSize:11}},(_nb>0||_ns)?[React.createElement('span',{key:'s',style:{color:_nclr,fontWeight:600}},_nb+'% '+_nlbl),' → ',React.createElement('span',{key:'a',style:{color:'var(--muted2)'}},_nact)]:'-')}else if(col.id==='avail')el=React.createElement('td',{key:col.id,className:'col-qty',style:{fontWeight:600}},x.available_qty);else if(col.id==='turnover'){const tc=x.turnover_days;el=React.createElement('td',{key:col.id,className:'col-qty',style:{fontWeight:600,color:tc!=null&&tc>30?'var(--danger)':tc!=null&&tc>15?'var(--warning)':'var(--text)'}},tc!=null?tc+'天':'∞')}else if(col.id==='price')el=React.createElement('td',{key:col.id,className:'col-price',style:{fontSize:12}},x.price?('¥'+Number(x.price).toFixed(1)):'-');else if(col.id==='stock_amount'){const sa=(x.available_qty||0)*(x.price||0);el=React.createElement('td',{key:col.id,className:'col-price',style:{fontWeight:600,fontSize:12}},sa?'¥'+sa.toLocaleString():'-')}else el=React.createElement('td',{key:col.id,className:'small muted',style:{fontSize:11}},'-');return el})
        ...
        if(isOpen&&batchLoading[bk]){
          ...
          batchData[bk].forEach(function(b,bi){
            ...
            const bcells=visCols.map(function(id){const col=INV_COLS[whType].find(function(c){return c.id===id});if(!col)return null;let el;if(col.id==='prod_date')el=React.createElement('td',{key:col.id,style:{fontSize:11}},b.prod_date||'-');else if(col.id==='exp_date')el=React.createElement('td',{key:col.id,style:{fontSize:11}},b.exp_date||'-');else if(col.id==='batch_days'){let td=0;if(b.exp_date&&b.prod_date)td=Math.round((new Date(b.exp_date)-new Date(b.prod_date))/86400000);el=React.createElement('td',{key:col.id,style:{fontSize:11}},td||'-')}else if(col.id==='eff_status'){let es='';if(b.exp_date&&b.prod_date){const cv=Math.round(((new Date()-new Date(b.prod_date))/86400000)/((new Date(b.exp_date)-new Date(b.prod_date))/86400000)*100);es=cv>=100?'⚫过期':(cv>=67?'✗否':(cv>40?'⚠️临近':'✓正常'))}el=React.createElement('td',{key:col.id,style:{fontSize:11,fontWeight:600,color:es==='✓正常'?'var(--success)':es==='⚠️临近'?'var(--warning)':es==='✗否'?'var(--danger)':es==='⚫过期'?'var(--accent-purple)':'var(--muted2)'}},es)}else if(col.id==='avail')el=React.createElement('td',{key:col.id,style:{fontWeight:600,fontSize:11}},b.qty);else if(col.id==='over_third'){let ot_st='-';if(b.exp_date&&b.prod_date){const td=Math.round((new Date(b.exp_date)-new Date(b.prod_date))/86400000);if(td>0){const cv=Math.round(((new Date()-new Date(b.prod_date))/86400000)/td*100);ot_st=cv>=100?'⚫已过期':(cv>33?'✗ 已超1/3':'正常')}}el=React.createElement('td',{key:col.id,style:{fontSize:11,color:ot_st.includes('已超')?'var(--danger)':ot_st.includes('过期')?'var(--accent-purple)':'var(--muted2)'}},ot_st)}else if(col.id==='note')el=React.createElement('td',{key:col.id,style:{fontSize:11,color:bcolor,fontWeight:600}},pct+'% '+note);else if(col.id==='warehouse'||col.id==='sku'||col.id==='name'||col.id==='barcode'||col.id==='channel'||col.id==='brand'){const v=col.id==='name'?x.product_name:(col.id==='brand'?(x.brand||'-'):(x[col.id]||'-'));el=React.createElement('td',{key:col.id,style:{fontSize:11}},v)}else if(col.id==='price')el=React.createElement('td',{key:col.id,style:{fontSize:11}},x.price?('¥'+Number(x.price).toFixed(1)):'-');else if(col.id==='month_in')el=React.createElement('td',{key:col.id,style:{fontSize:11,color:'var(--muted2)'}},'-')
            ...
            else if(col.id==='stock_amount'){const _amt=b.qty*(x.price||0);el=React.createElement('td',{key:col.id,style:{fontSize:11,fontWeight:600,color:_amt>0?'var(--text)':'var(--muted2)'}},_amt>0?'¥'+_amt.toLocaleString():'-')}

...
### 清洗导入解析治本(多列表被解析成单列的完整链路)
- **xlsx read_only 维度误判**: 非标准 xlsx(无默认样式) read_only 下 max_row/column=1×1 → 只读表头1列0行; 改小文件普通模式+取第一个有数据 sheet(实测 48列9行 → 48列8行)
- **CSV 分隔符兜底探测**: Sniffer 对竖线/中文逗号失效(实测仍单列) → 手动候选探测(\t;|，、,)+列数一致性校验; CSV 表头空列过滤

### 映射页/导入流程 UI/UX 重构(多轮)
- 步骤条 s=0 隐藏+识别中居中 / '字段映射'→'映射字段' / 统计多行主次排版
- 映射字段按导入类型过滤(TARGET_FIELDS 专属字段集, 识别目标校验) + 字段集补齐(商品/库存品牌单价备注/出入库 21 字段含商品属性)
- 映射卡片分层(模板工具条弱化/列映射主分区/行 24 圆角) / 底部按钮统一
- 商智日销/京东采购单按钮去除(暂不用) / 统计行去除 / 进销存表头冗余列计数去除
- **模版管理+自定义字段迁入清洗页锤子菜单**(仅映射步骤显示, 底部弹窗, 事件联动映射页); **表格列状态映射**按钮按类型+步骤条件显示
- **底部弹窗统一看板'还有N条'同款**(透明遮罩+底部居中+18px标题+主色圆角22关闭块)
- **portal 修复**: 锤子菜单玻璃容器(backdrop-filter)创建包含块致 fixed 弹窗渲染菜单内 → createPortal(body) 根治

### 架构: 列值映射通用化 + 动态自定义列数据链路
- **column-value-map 通用接口**(列名→值档位, 兼容旧 order_status_map; 未来拓展其他列筛选), 迁移清洗页锤子菜单'表格列状态映射'弹层(渠道级隔离); 规则页状态映射卡移除
- **动态自定义列方案B评估后取舍**: ext_json 数据链路(6表幂等补列/清洗写入自定义字段保全/接口返回/custom-columns 配置)保留为基础设施; 前端动态列 UI 因低频+一次性补列更划算而回退(30分钟补列 vs 常驻功能成本)

### 设计系统收敛(hue 设计方法论, P0/P1/P2)
...
- 漏斗总订单 bar 占比统一: xAxis max 显式=总订单×1.15(排查: 两渠道比例实际相同 2.16vs2.15, 差异为 echarts max 自动取整)
- 规则页 severity/启用停用/mode pill 统一胶囊(订单状态同款, 移除内联矮圆覆盖)

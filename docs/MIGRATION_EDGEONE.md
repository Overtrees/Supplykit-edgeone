...
- **Phase 4 🔄（09-05 进展）**：前端**已切 Makers**（VITE_API_BASE_URL=同源 edgeone.cool, 登录+看板端到端验证通过）；**同源免签实测**: 签名 cookie(3h)会话内浏览器同源请求直达函数——3h 预览可跑前端全功能一比一；**生产公开访问待解决**: 自定义域名(大陆 ICP 备案, 用户暂无域名)或确认海外免签(报错提示 Global MLC excluded)
...
## 7. Phase 状态更新(2026-09-06)——迁移完成, 生产公开访问达成
- **Phase0-3 全部完成**: 环境/语法兼容、TiDB 建表+seed、方案B 原生重构九路由、前端切换
- **Phase4 生产公开访问达成**: 新项目 makers-8gstkvheqm2c(Area=overseas 免备案) + 自定义域名 supplykit.top 免签上线(原"待自定义域名备案"已解决——overseas 区免备案)
- **契约缺口清零**: A/B/C 三组 39 端点补齐(cleansing/tasks/seed/exports/purchase-orders/disposals 等)
- **业务逻辑对齐 PA**: 规则引擎/滞销多因素/库存联动/采购供应商参数/进销存当月/批次摘要/周期趋势/品牌维度等 20+ 项逐页对齐
- **免费额度优化**: 分析缓存+中央失效+索引+轮询降频; RU 实测 18% 用量, 余量充足
- **剩余**: 定时任务自动触发验证(每日 cron 已配, 待首触发); 真实数据迁移(演示 18.7 万订单数据已在); 待部署 commit 66321aad(单日构建上限恢复后)

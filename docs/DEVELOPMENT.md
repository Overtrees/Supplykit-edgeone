...
### 15.28 清洗导入全链路 + 设计系统收敛 + 图表坑(2026-09-10)
- **解析链路两坑**: ①xlsx read_only 对非标准文件(无默认样式/维度信息) max_row/column 误判 1×1 → 只读表头1列0行; 小文件普通模式+取第一个有数据 sheet ②CSV Sniffer 对竖线/中文逗号失效 → 手动候选分隔符探测+列数一致性校验。**教训: 上传解析必须用真实用户文件实测**(48列xlsx 暴露)
